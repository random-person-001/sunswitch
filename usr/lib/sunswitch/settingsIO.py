...
settingsDefault = {"update":True,"date":True,"resIndex":0,"typeIndex":0,"green":True}
        ...
        try:        
            with open(path, 'r') as f:
                ...
                if i < len(settingsOrder):
                    ...
                    rewrite = True
            if not rewrite:
                if (type(settings[settingsOrder[0]]) != type(True)) or (type(settings[settingsOrder[1]]) != type(True)) or (type(settings[settingsOrder[4]]) != type(True)):
                    print "First two items weren't bools!  Corrupt file!"
                    rewrite = True
