...
def findY(res): #Helps put the date in the corner where it should be (maybe)
    from gi.repository import Gtk
    screen = Gtk.Window().get_screen()
    if screen.get_n_monitors() > 1: #Multimonitor display; don't bother with date
        return "multiple monitors"
    ...
    h = firstMon.height
    s = float(w/res)
    return float( float(w-h)/2.0 + h ) * s
    #return firstMon.width/firstMon.height
...
def recolorAndDateMain(original, saveAs):#oldOneToo):
    ...
    if s.imageRes > 1025 and s.insertDate: #Only bother inserting the date visibly if it's high enough rez
        ...
        bottomMargin = 10
        print findY(s.imageRes)
        print "Y: " + str(width-20-bottomMargin) + " to " + str(width-bottomMargin)
        print "X: " + str(s.imageRes-300) + " to " + str(s.imageRes)

        #ratio = monitorRatio()
        #scale = width/s.imageRes

        img[width-20-bottomMargin:width-bottomMargin, s.imageRes-300:s.imageRes] = date
            

    #if oldOneToo:    A lazy settings thing.  Figured it wasn't worth it.
...
def doStuff(settings=()):
    ...
    print gsettings.set_string(KEY, "file://" + s.finalFile)
